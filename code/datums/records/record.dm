...
/datum/record/locked/New(
	...
	datum/mind/mind_ref,
	age_chronological = 18,	// EXOSTATION EDIT ADDITION - CHAR_LORE : chronological age
)
	. = ..()
	src.locked_dna = locked_dna
	src.mind_ref = WEAKREF(mind_ref)
	src.age_chronological = age_chronological	// EXOSTATION EDIT ADDITION - CHAR_LORE : chronological age
...
/datum/record/crew/proc/get_rapsheet(alias, header = "Rapsheet", description = "No further details.")
	...
	final_paper_text += "Name: [name]<br>Gender: [gender]<br>Age: [age]<br> Chronological Age: [age_chronological]<br>" // EXOSTATION EDIT CHANGE - CHAR_LORE : ajout de  Chronological Age: [age_chronological]<br>
	if(alias != name)
