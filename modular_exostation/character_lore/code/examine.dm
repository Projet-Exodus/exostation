...
/mob/living/proc/get_extended_description_href(input_text)
	...
	return "<a href=byond://?src=[REF(src)];full_desc=1;examine_time=[world.time]'>[input_text]</a>"
...
/mob/living/proc/get_species_description_href(input_text)
	...
	return "<a href=byond://?src=[REF(src)];species_info=1;examine_time=[world.time]'>[input_text]</a>"

/mob/living/proc/get_exploitables_href(input_text)
	return "<a href=byond://?src=[REF(src)];exploitables=1;examine_time=[world.time]'>[input_text]</a>"
...
/mob/living/proc/compile_examined_text(short_desc, extended_desc, headshot, ooc_notes)
	...
	if (length(ooc_notes))
		full_examine += span_slightly_larger(separator_hr("<em>OOC Notes</em>")) // EXXOSTATION EDIT CHANGE - TRADUCTION : à faire
...
// Override code/game/atom_examine.dm
/mob/living/carbon/human/examine_title(mob/user, thats = FALSE)
	...
	if(!species_visible)
		...
		species_name_string = ", <EM>[dna.species.name]</EM>"

	. += species_name_string

// Override code/modules/mob/living/carbon/human/human.dm
...
/mob/living/carbon/human/Topic(href, href_list)
	...
	if (href_list["full_desc"])
		...
		var/species_name = src.dna.species.name
		var/species_desc = src.dna.species.get_species_description()
		...
		var/the_goss = src.dna.features["exploitables"] ? src.dna.features["exploitables"] : "Vos contacts n'ont reçu aucun renseignement particulier concernant cette personne."
...
// Override code/modules/mob/living/silicon/robot/robot.dm
...
// Override code/modules/mob/living/silicon/ai/ai.dm
