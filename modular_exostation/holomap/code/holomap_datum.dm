...
/datum/station_holomap/New()
	. = ..()
	cursor = image('modular_exostation/holomap/icons/8x8.dmi', "you")
...
/datum/station_holomap/proc/generate_legend(list/overlays_to_use = list())
	...
	for(var/list/overlay_name as anything in overlays_to_use)
		...
		if(overlay_name in disabled_overlays)
			var/image/disabled_marker = image('modular_exostation/holomap/icons/8x8.dmi', "legend_cross")
...
/datum/station_holomap/proc/initialize_holomap_bogus()
	...
	var/image/legend = image('modular_exostation/holomap/icons/64x64.dmi', "notfound")
	legend.pixel_x = 192
	legend.pixel_y = 224
